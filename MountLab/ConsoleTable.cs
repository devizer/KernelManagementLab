...
namespace ConsoleTables
{
    public class ConsoleTable
    {
        ...
        public static ConsoleTable From<T>(IEnumerable<T> values)
        {
            ...
            var itemType = copy.Count > 0 ? copy[0].GetType() : typeof(T);
            var columns = copy.Count > 0 ? GetColumns(copy[0].GetType()) : GetColumns<T>();
                
            table.AddColumn(columns);

            foreach (var propertyValues in values.Select(value => columns.Select(column => GetColumnValue2(itemType, value, column) )))
        ...
        public string ToStringAlternative()
        {
            ...
            foreach (var row in results)
            {
                if (isFirst) builder.AppendLine(dividerPlus);
                isFirst = false;
                builder.AppendLine(row);
            }
            builder.AppendLine(dividerPlus);
        ...
        private static IEnumerable<string> GetColumns(Type itemType)
        {
            return itemType.GetProperties().Select(x => x.Name).ToArray();
        }

        private static object GetColumnValue2(Type itemType, object target, string column)
        {
            return itemType.GetProperty(column).GetValue(target, null);
        }
    }

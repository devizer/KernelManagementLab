...
namespace Universe.Benchmark.DiskBench
{
    public class ReadonlyDiskBenchmark : IDiskBenchmark
    {
            ...
            try
            {
                ...
                foreach (var fileInfo in WorkingSet)
                {
                    fileInfo.Stream = new FileStream(fileInfo.FullName, FileMode.Open, FileAccess.Read, FileShare.Read,
                        Parameters.RandomAccessBlockSize, GenericReadingFileStreamOptions);
        ...
        private void SeqRead()
        {
            ...
            {
                CancelIfRequested();
                using (FileStream fs = new FileStream(fileInfo.FullName, FileMode.Open, FileAccess.Read, FileShare.ReadWrite, buffer.Length, GenericReadingFileStreamOptions))
                {
                    long len = 0;
...
{string.Join(Environment.NewLine, _NormalizerMountPaths.Select(x => $" {{{x}}}"))}");
        ...
        bool CanReadFile(string fullName)
        {
            try
            {
                ...
                using (FileStream fs = new FileStream(fullName, FileMode.Open, FileAccess.Read, FileShare.Read, 1, GenericReadingFileStreamOptions))
                {
                    fs.Read(buffer, 0, 1);
                    return true;
                }
